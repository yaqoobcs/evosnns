...
def plot(x, fname):
    ...
    # skipVal=0
    # clockVal=350
    fig, (ax6, ax7, ax8, ax9, ax10, ax11, ax13) = plt.subplots(
        7, 1, sharex=False, sharey=False, figsize=(gWidth, 7)
    )
    ...
    N3 = x[:, 10][skipVal:clockVal]
    N4 = x[:, 11][skipVal:clockVal]
    ...
    lwid = 2
    ax6.plot(clock, A, "k-", linewidth=lwid)
    ax6.plot(clock, B, "m-", linewidth=lwid)
    ax6.plot(clock, C, "r-", linewidth=lwid)
    ax6.plot(clock, D, "b", linewidth=lwid)
    ax6.plot(clock, E, "c-", linewidth=lwid)
    # ax6.plot(clock,F, 'g-', linewidth=lwid)
