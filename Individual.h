#pragma once
#include <fstream>
#include <iostream>
#include <sstream>
#include <utility>
#include <vector>
#include "Neuron.h"
#include "Parameters.h"
#include "UtilityFunctions.h"


using namespace std;
extern Parameters params;

class Individual {
private:
public:
    double long fitness;
    double long rewardn, penaltyn;
    double long fdr;
    double long precision;
    double reward, penalty;
    double absWeightSum;
    int rank, totalCorrPatterns;
    std::vector<std::vector<double>> indMatrix;
    // double** indMatrix;
    vector<Neuron> inputNeurons;
    vector<Neuron> interNeurons;
    vector<Neuron> outputNeurons;
    int noOfInputs, noOfinterNeurons, noOfOutputNeurons;
    int noOfNodesInNetwork;
    vector<double> gaussNoiseVector;
    vector<string> missIdentifiedPatterns;

    Individual(void);
    ~Individual(void);

    void initializeMatrix(int size);
    vector<vector<double>> getIndividualMatrix();
    void printIndividualMatrix(int gNo, int irun);
    void printIndividualMatrix(int gNo, int iRun, const std::string &saving_dir, const std::string &orgi_file_name);
    void outputNetworkActivity(const std::string &output_file);
    void readIndividualMatrix(string fname);
    void makeRandomIndividual(NeuronType ntype, int noInputs, int nointerNeurons, int noOutputs);
    void makeIndividualWithFixedInputOutputConnections(NeuronType ntype, int noInputs, int nointerNeurons,
                                                       int noOutputs);
    void makeIndividualWithFixedInputOutputConnections(NeuronType ntype, int noInputs, int nointerNeurons,
                                                       int noOutputs, std::string file_to_load);
    void loadEvolvedTop(NeuronType ntype, int noInputs, int nointerNeurons, int noOutputs);
    void loadIndividualNetwork(NeuronType ntype, int noInputs, int nointerNeurons, int noOutputs, string name);
    void makeIndividualWithFixedLoops(NeuronType ntype, int noInputs, int nointerNeurons, int noOutputs);
    void replicate();
    void replicateWithGaussianNoise();
    void replicateInputconnection_WithGaussianNoise();
    void replicateInput_and_Switch_connections();
    void replicateinterConnections();
    void randomizeinterconnections();
    void replicateExceptLoops();
    void replicateByOnlyReducingWeights();
    void deleteWeight();
    void mutateWeight();
    void mutateSign();
    void removeLowWeights();
    void deleteIndMatrix();
    void networkStep(long long int stepNo);
    void activateOutput(long long int stepNo);
    void setInput(char inputSignal, long int index);
    void setGap();
    void sumOfConnectionWeights();
    void resetIndividual();
};
