...
double ComputationalUnit::resetVoltage() { return Vreset; }
...
double ComputationalUnit::updateVoltage(double currentVoltage, double excitatory_conductance,
                                        double inhibitory_conductance, double Adaptation) {
    double voltageChange;
    if (nModel == LIF) {
        voltageChange = ((Vresting - currentVoltage) * inv_taum +
                         (excitatory_conductance * (e_rev_E - currentVoltage) +
                          inhibitory_conductance * (e_rev_I - currentVoltage) + i_offset) *
                                 inv_C) *
                        params.timeStep;
    } else if (nModel == ADX) {
        voltageChange = ((Vresting - currentVoltage + DeltaT * exp((currentVoltage - Vthreshold) / DeltaT)) * inv_taum +
                         (excitatory_conductance * (e_rev_E - currentVoltage) +
                          inhibitory_conductance * (e_rev_I - currentVoltage) + i_offset - Adaptation) *
                                 inv_C) *
                        params.timeStep;
    } else if (nModel == IZH) {
        voltageChange = (0.04 * pow(currentVoltage, 2) + 5 * currentVoltage + 140 - Adaptation +
                         (excitatory_conductance * (e_rev_E - currentVoltage) +
                          inhibitory_conductance * (e_rev_I - currentVoltage) + i_offset) *
                                 inv_C) *
                        params.timeStep;
